...
export const CharacterProfileModal: React.FC = () => {
  const { profile, isOpen, closeProfile } = useCharacterStore();

  if (!isOpen) return null;

  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-sm" onClick={closeProfile}>
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.95 }}
          className="bg-terra-dark border border-gray-700 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded shadow-2xl flex flex-col"
          ...
          <div className="flex justify-between items-center p-3 border-b border-gray-800 bg-black/20">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1 text-terra-text font-bold text-lg">
                {profile.alignment.side === 'synthesis' && <span className="text-green-500">☣</span>}
                {profile.alignment.side === 'relicts' && <span className="text-yellow-500">🏺</span>}
                {profile.alignment.side === 'shadow' && <span className="text-gray-500">👤</span>}
                {profile.alignment.side === 'pulse' && <span className="text-red-500">⚡</span>}
                {profile.alignment.side === 'outcast' && <span className="text-orange-500">🚫</span>}
                <span>{profile.name}</span>
                <span className="text-gray-500 text-sm font-normal">[{profile.level}]</span>
              </div>
            ...
            <div className="md:col-span-3 space-y-6">
              <div>
                ...
                <div className="space-y-1">
                  <StatRow label="Сила" value={profile.stats.strength} />
                  <StatRow label="Ловкость" value={profile.stats.agility} />
                  <StatRow label="Интуиция" value={profile.stats.intuition} />
                  <StatRow label="Воля" value={profile.stats.will} />
                  <StatRow label="Конституция" value={profile.stats.constitution} />
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-6 flex flex-col items-center">
              ...
              <div className="w-full max-w-xs mb-4 space-y-1">
                <div className="relative h-4 bg-gray-900 rounded border border-gray-700 overflow-hidden group cursor-help">
                  <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-red-900 to-red-700 transition-all duration-500" style={{ width: `${(profile.hp.current / profile.hp.max) * 100}%` }} />
                ...
                <div className="relative h-2 bg-gray-900 rounded border border-gray-700 overflow-hidden" title="Энергия">
                  <div className="absolute top-0 left-0 h-full bg-yellow-500/80 transition-all duration-500" style={{ width: `${(profile.energy.current / profile.energy.max) * 100}%` }} />
                </div>
              </div>
              ...
              <div className="relative w-full max-w-[400px] aspect-[4/5] bg-gradient-to-b from-gray-900/50 to-transparent rounded-2xl border border-gray-800/50 p-4">
                ...
                <div className="flex h-full gap-2 relative z-10">
                  ...
                  <div className="flex flex-col justify-between py-4 w-12 sm:w-16 items-center">
                    <EquipmentSlot item={profile.equipment.helmet} placeholder="Шлем" />
                    <EquipmentSlot item={profile.equipment.earrings} placeholder="Серьги" />
                    <EquipmentSlot item={profile.equipment.amulet} placeholder="Амулет" />
                    <EquipmentSlot item={profile.equipment.weapon} placeholder="Оружие" />
                    <EquipmentSlot item={profile.equipment.ring1} placeholder="Кольцо" />
                    <EquipmentSlot item={profile.equipment.pants} placeholder="Штаны" />
                  </div>
                  ...
                  {/* Right Slots */}
                  <div className="flex flex-col justify-between py-4 w-12 sm:w-16 items-center">
                    <EquipmentSlot item={profile.equipment.armor} placeholder="Броня" />
                    <EquipmentSlot item={profile.equipment.belt} placeholder="Пояс" />
                    <EquipmentSlot item={profile.equipment.gloves} placeholder="Перчатки" />
                    <EquipmentSlot item={profile.equipment.shield} placeholder="Щит" />
                    <EquipmentSlot item={profile.equipment.ring2} placeholder="Кольцо" />
                    <EquipmentSlot item={profile.equipment.boots} placeholder="Обувь" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-3 space-y-6">
              <div>
                ...
                <div className="text-xs text-gray-400">
                  ...
                  <div className="text-gray-300 mb-2">{profile.registrationDate}</div>
                  <div className="mb-1">Гражданство:</div>
                  <div className="text-green-500">{profile.citizenship}</div>
                </div>
              </div>
              ...
              {profile.clan && (
                <div>
                  ...
                  <div className="flex items-start gap-2 text-xs">
                    ...
                    <div>
                      <div className="font-bold text-terra-gold hover:underline cursor-pointer">{profile.clan.name}</div>
                      <div className="text-gray-500 mt-0.5">{profile.clan.rank}</div>
                    </div>
                  </div>
                </div>
              )}
