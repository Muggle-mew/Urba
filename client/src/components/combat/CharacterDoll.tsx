    ...
    <div className="flex flex-col items-center w-full h-full">
      ...
      <div className="w-full mb-2 shrink-0">
        <div className={clsx("flex items-baseline gap-2 mb-1", isRight ? "flex-row-reverse" : "flex-row")}>
          <span className="font-bold text-terra-text text-sm sm:text-base truncate">{name}</span>
      ...
      {/* Doll & Slots Grid */}
      <div className="relative w-full flex-1 min-h-0 bg-terra-dark/30 rounded-lg border border-gray-800 overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-0">
           ...
           <div className="h-full w-full max-w-md bg-gradient-to-b from-gray-700 to-transparent opacity-10 rounded-full blur-2xl absolute"></div>
           <img 
             src={imageSrc} 
             alt="avatar" 
             className={clsx("h-full w-full object-contain drop-shadow-2xl grayscale hover:grayscale-0 transition-all duration-500", isRight && "scale-x-[-1]")}
           />
        </div>

        <div className="relative h-full flex justify-between z-10 pointer-events-none p-2 sm:p-4">
            {/* Left Column Slots */}
            <div className="flex flex-col justify-between h-full py-2 gap-1 pointer-events-auto">
                <Slot label="Шлем" />
                <Slot label="Серьги" />
                <Slot label="Амулет" />
                <Slot label="Оружие" />
