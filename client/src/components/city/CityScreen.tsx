...
import { SewersModal } from '../sewers/SewersModal';
...
export const CityScreen: React.FC<CityScreenProps> = ({ onNavigateToCombat }) => {
  ...
  const [showSewersModal, setShowSewersModal] = useState(false);
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-gray-900">
      ...
      <div className="flex-1 flex h-full">
        ...
        <div className="w-3/4 relative h-full bg-black border-r border-gray-800 overflow-hidden group">
          ...
          >
            ...
            {/* Sewers (Bottom Right) */}
            <button 
              onClick={() => setShowSewersModal(true)}
        ...
        <div className="w-1/4 bg-gray-900 flex flex-col gap-2 p-2 lg:gap-4 lg:p-4 border-l border-gray-800 shadow-2xl z-20 overflow-hidden">
          
          {/* 1. To Battle */}
          <button 
            onClick={onNavigateToCombat}
            className="flex-1 min-h-0 bg-gradient-to-br from-red-900 to-red-950 border-2 border-red-700 rounded-xl flex flex-col items-center justify-center gap-2 lg:gap-4 group hover:border-red-500 hover:from-red-800 hover:to-red-900 transition-all shadow-lg hover:shadow-red-900/20"
          >
            <Swords className="w-8 h-8 lg:w-16 lg:h-16 text-red-500 group-hover:text-red-300 transition-colors group-hover:scale-110 duration-300 shrink-0" />
            <span className="text-sm lg:text-2xl font-bold text-red-100 uppercase tracking-wider group-hover:text-white truncate">В БОЙ</span>
          </button>

          {/* 2. Travel */}
          <button 
             onClick={() => setShowTravelModal(true)}
             className="flex-1 min-h-0 bg-gradient-to-br from-blue-900 to-blue-950 border-2 border-blue-700 rounded-xl flex flex-col items-center justify-center gap-2 lg:gap-4 group hover:border-blue-500 hover:from-blue-800 hover:to-blue-900 transition-all shadow-lg hover:shadow-blue-900/20"
          >
            <Navigation className="w-8 h-8 lg:w-16 lg:h-16 text-blue-500 group-hover:text-blue-300 transition-colors group-hover:scale-110 duration-300 shrink-0" />
            <span className="text-sm lg:text-2xl font-bold text-blue-100 uppercase tracking-wider group-hover:text-white truncate">Перемещение</span>
          </button>

          {/* 3. Laboratory */}
          <button 
             className="flex-1 min-h-0 bg-gradient-to-br from-green-900 to-green-950 border-2 border-green-700 rounded-xl flex flex-col items-center justify-center gap-2 lg:gap-4 group hover:border-green-500 hover:from-green-800 hover:to-green-900 transition-all shadow-lg hover:shadow-green-900/20"
          >
            <FlaskConical className="w-8 h-8 lg:w-16 lg:h-16 text-green-500 group-hover:text-green-300 transition-colors group-hover:scale-110 duration-300 shrink-0" />
            <span className="text-sm lg:text-2xl font-bold text-green-100 uppercase tracking-wider group-hover:text-white truncate">Лаборатория</span>
          </button>
      ...
      {/* Sewers Modal */}
      <SewersModal 
        isOpen={showSewersModal} 
        onClose={() => setShowSewersModal(false)} 
      />
    </div>
  );
};
